...
// topological ray shooting for material interface
void topo_ray_shooting(
        const std::vector<std::array<double, 3>> &pts,
        const std::vector<std::array<size_t, 4>> &tets,
...
// find extremal edge for each component (material interface)
void find_extremal_edges(
        const std::vector<std::array<double, 3>> &pts,
        const std::vector<MI_Vert> &MI_verts,
        const std::vector<PolygonFace> &MI_faces,
...
// return a list of sorted vertex indices {v_id, i1, i2, ..., u_id}
void compute_edge_intersection_order(const simplicial_arrangement::MaterialInterface<3>& tet_cut_result, size_t v, size_t u,
                                        std::vector<size_t> &vert_indices);
...
// find the two faces passing v1 and v2, v1->v2 is part of a tet edge (material interface)
